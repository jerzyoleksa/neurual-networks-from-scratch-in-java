...
/*		double[][] errors = np.subtract(outputs, neuralOutputs);
	...
	public static void main(String[] args) {
		...
		ArrayList<double[][]> updatedWeights = new MyNeuralNetXOR().train(inputs, np.T( outputs ), weightsLayer1, weightsLayer2, 60000);
		weightsLayer1 = updatedWeights.get(0);
		weightsLayer2 = updatedWeights.get(1);
		//MatrixPrinter.printMatrix(updatedWeights);
		
		double[][] inp = {{0,1,0}};
		ArrayList<double[][]> predicted = new MyNeuralNetXOR().predict(inp, weightsLayer1, weightsLayer2);
